/* version 160623
  ...
  tested in FIJI 2.0.0-rc-49/1.51a on Linux Mint 64-bit
...
var original, originalID, name, channel, altDown=false, shiftDown=false;//global variables about image
...
var Periost, Endoost, original, scalar, units, TA, CA, Cx, Cy, Ix, Iy, Imax, Imin, Zx, Zy, Xmaxrad, Ymaxrad, xmin, ymin, xmax, ymax, J, Zp, maxRad;
...
//******************************* MACROS *****************************************************
/*macro "reInstall MMJ [n9]" {
	run("Install...", "install="+IJPath+"/macros/bones/MomentMacroJ_160623.ijm");
}*/
...
macro "Install new version" {
	...
	sep=File.separator;
	macroFile=File.openDialog("Navigate to MomentMacroJ.ijm source file");
	MMJFile=macroDir+"MomentMacroJ.ijm";
	if (macroFile!=MMJFile || File.exists(macroFile)) {
		File.copy(macroFile, MMJFile);
		AutoRunDir=macroDir+"AutoRun"+sep;
		AutoRunFile=AutoRunDir+"install_MMJ.ijm";
		if (!File.exists(AutoRunDir)) File.makeDirectory(AutoRunDir);
		script="macroDir=getDirectory(\"macros\"); run(\"Install...\", \"install=\"+macroDir+\"MomentMacroJ.ijm\");";
		File.saveString(script, AutoRunFile);
		if (File.exists(AutoRunFile)) {
			//print("Autoinstallation 'install_MMJ.ijm' file succesfully created in "+AutoRunFile);
			runMacro(AutoRunFile);
			exit ("MomentMakroJ new version succesfully installed.");
		}
...
macro "Batch measure [n0]" {  //batch proceed all images in the folder
	...
	if (!File.isDirectory(option[5])) {
		option[5]=getDirectory("Choose a measurements output directory"); // set the output directorzy if not yet difined
		option[4]=false;
		setOptions(option);
		}
...
macro "MomentMacroJ [n1]" {
	altDown=isKeyDown("Alt"); //was Alt pressed? n1 with alt should force a new measure instead of trying to recover older one.
	shiftDown=isKeyDown("Space");
	setKeyDown("none");
	MomentMacroJ();
}
...
if (!shiftDown) flagZIP=openZipTif();	//if not shift pressed try to open corresponding tif, dcm or zip with selections from original or destination folder.
else flagZIP=false; //if shift pressed then starts measurement from begin
...
//calculate CA
	getStatistics(CA); //get CA of the selection
	//CA= Sn*sqr(scalar); //this gives slightly different values at lower number orders than getStatistics method
//calculate TA
	if (Periost==ostDef[0]) taSel=Endoost; //if Periost is null
	else if (Periost==ostDef[12]) taSel=ostDef[1]+"Periost"; //if Periost is Threshold
	else taSel=Periost; //all other cases
	selectROI(taSel);
	//stop();
